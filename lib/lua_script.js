...
const SCRIPT_ROOT = __dirname + '/../qless-lua/qless.lua';

let scriptText = null;
function getScriptText() {
  if (!scriptText) scriptText = fs.readFileSync(SCRIPT_ROOT);
  return scriptText;
}
...
class LuaScript {
  constructor(name, redis) {
    this.name = name;
    this.redis = redis;
    this.scriptContents = getScriptText();
    this.sha = crypto.createHash('sha1').update(this.scriptContents).digest('hex');
  }

  reload(cb) {
    this.redis.script('load', this.scriptContents, makeCb(cb, res => {
      this.sha = res;
      return cb();
    }));
  }

  ...
  async monitor() {
    ...
    while (!this.stopped) {
      ...
      for (const child of this.pool.workers) {
        logger.debug(`Checking memory for ${child.worker.pid}`);
        // eslint-disable-next-line no-await-in-loop
        const { memory } = await usage.lookupAsync(child.worker.pid);

        logger.debug(`Process ${child.worker.pid} using ${bytes(this.memory.max)}`);
        if (memory > this.memory.max) {
          logger.warn(`Killing child ${child.worker.pid} for exceeding memory limit ${bytes(memory)} (${memory}) > ${bytes(this.memory.max)} (${this.memory.max})`);

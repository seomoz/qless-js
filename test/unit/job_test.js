...
describe('qless.Job', () => {
  ...
  /*
  it("Raises an error if the class doesn't have the method", function *() {
    qlessClient.queue('foo').put('foo.Foo', {}, {jid: 'jid'})
    qlessClient.queue('foo').pop().process()
  ...
  it("Rasises an error if the relevant function's not static", function *() {
    qlessClient.queue('nonstatic').put(Foo, {}, {jid: 'jid'})
    qlessClient.queue('nonstatic').pop().process()
    job = qlessClient.jobs['jid']
    self.assertEqual(job.state, 'failed')
    if PY3:
  ...
  xit("We can set a job's priority", function *() {
    yield qlessClient.queue('foo').put('Foo', {}, {jid: 'jid', priority:0})
  ...
  xit("Exposes a queue object", function *() {
    qlessClient.queue('foo').put('Foo', {}, {jid: 'jid'})
    self.assertEqual(qlessClient.jobs['jid'].queue.name, 'foo')
  });


  xit("Exposes the class for a job", function *() {
    qlessClient.queue('foo').put(Job, {}, {jid: 'jid'})
    self.assertEqual(qlessClient.jobs['jid'].klass, Job)
  });


  xit("Exposes the ttl for a job", function *() {
    qlessClient.config['heartbeat'] = 10
    qlessClient.queue('foo').put(Job, {}, {jid: 'jid'})
    qlessClient.queue('foo').pop()
    self.assertTrue(qlessClient.jobs['jid'].ttl < 10)
    self.assertTrue(qlessClient.jobs['jid'].ttl > 9)
  });


  xit("Raises an attribute error for nonexistent attributes", function *() {
    qlessClient.queue('foo').put(Job, {}, {jid: 'jid'})
    self.assertRaises(AttributeError, lambda: qlessClient.jobs['jid'].foo)
  });


  xit("Exposes the cancel method", function *() {
    qlessClient.queue('foo').put('Foo', {}, {jid: 'jid'})
  ...
  xit("Exposes a way to tag and untag a job", function *() {
    qlessClient.queue('foo').put('Foo', {}, {jid: 'jid'})
  ...
  it("Able to advance a job to another queue", function *() {
    qlessClient.queue('foo').put('Foo', {}, {jid: 'jid'})
    qlessClient.queue('foo').pop().complete('bar')
  ...
  it("Exposes a depend, undepend methods", function *() {
    qlessClient.queue('foo').put('Foo', {}, {jid: 'a'})
    qlessClient.queue('foo').put('Foo', {}, {jid: 'b'})
    qlessClient.queue('foo').put('Foo', {}, {jid: 'c', depends=['a']})
